...
MAX_ITEMS_VIDEOS="${MAX_ITEMS_VIDEOS:-0}"   
MAX_ITEMS_SHORTS="${MAX_ITEMS_SHORTS:-2000}"
MAX_ITEMS_STREAMS="${MAX_ITEMS_STREAMS:-0}"
...
# ⚠️ Ya NO agregamos --playlist-end aquí (será por TAB en list_rows_tab)
...
# Resuelve límite efectivo por TAB (prioridad: por-tab > global)
limit_for_tab() {
  local tab="$1" lim=0
  case "$tab" in
    videos)  lim="${MAX_ITEMS_VIDEOS}";;
    shorts)  lim="${MAX_ITEMS_SHORTS}";;
    streams) lim="${MAX_ITEMS_STREAMS}";;
  esac
  # Si no se definió por tab, cae al global
...
# Lista filas (TSV) de una tab (id, title, url, playlist_title, live_status) con límite por TAB
list_rows_tab() {
  ...
  # Límite por tab
  local LIM; LIM="$(limit_for_tab "$tab")"
  local -a extra_args=()
  if [[ "$LIM" =~ ^[0-9]+$ ]] && (( LIM > 0 )); then
    extra_args+=(--playlist-end "$LIM")
  fi

  # Con --flat-playlist imprimimos directamente los campos de los ítems de la tab
  YT "${extra_args[@]}" \
     --print '%(id)s\t%(title)s\t%(webpage_url)s\t%(playlist_title)s\t%(live_status)s' "$url" \
    | awk -F'\t' '
        BEGIN{OFS="\t"}
        {
...
dump_tab_tsv() {
  ...
  log "OK -> $(basename "$out") (${n} filas) [limit=$(limit_for_tab "$tab")]"
}
...
Opciones:
  ...
  --only PREFIX1,...     Procesa solo estos prefijos (keys de CHANNELS_DEFAULT)
  --out DIR              Directorio de salida (por defecto: ${OUT_DIR})
  --max N                Máximo global de ítems por tab (fallback)
  --max-videos N         Máximo específico para videos
  --max-shorts N         Máximo específico para shorts (default: ${MAX_ITEMS_SHORTS})
  --max-streams N        Máximo específico para streams
  --client C             Cliente extractor (android|ios|web). Por defecto: ${EXTRACTOR_CLIENT}
  -h|--help              Ayuda

Variables de entorno útiles:
  YT_BIN, COOKIES_FILE, OUT_DIR, EXTRACTOR_CLIENT,
  MAX_ITEMS, MAX_ITEMS_VIDEOS, MAX_ITEMS_SHORTS, MAX_ITEMS_STREAMS, TABS_ARG

Ejemplos:
  TABS_ARG="videos,shorts" ./$(basename "$0")
  ./$(basename "$0") --tabs videos,streams --only dalas,lasalseria --max 200
  EXTRACTOR_CLIENT=ios ./$(basename "$0") --max-shorts 2000
EOF
}
...
while (( $# )); do
  case "$1" in
    ...
    --max-videos)  shift; MAX_ITEMS_VIDEOS="${1:-$MAX_ITEMS_VIDEOS}";;
    --max-shorts)  shift; MAX_ITEMS_SHORTS="${1:-$MAX_ITEMS_SHORTS}";;
    --max-streams) shift; MAX_ITEMS_STREAMS="${1:-$MAX_ITEMS_STREAMS}";;
...
log "Cliente: ${EXTRACTOR_CLIENT} | Max(global): ${MAX_ITEMS} | Max(videos): ${MAX_ITEMS_VIDEOS} | Max(shorts): ${MAX_ITEMS_SHORTS} | Max(streams): ${MAX_ITEMS_STREAMS} | Cookies: $( [[ -f "$COOKIES_FILE" ]] && echo 'sí' || echo 'no' )"
log "Tabs: ${TABS_ARG}"
log "Prefijos: ${PREFS[*]}"
